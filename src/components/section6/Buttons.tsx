            ...
            <div className='flex flex-wrap gap-3 items-center'>
                {
                    categories.map((item, index) => (
                        <Button key={index} variant={item.active ? "default" : "gray"} className='px-[20px] py-[10px] duration-200 text-[14px] font-medium font-dm_sans' onClick={() => handleClick(index)}>
                            {item.title}
                        </Button>))}
            </div>
            <Button className='px-[20px] py-[10px]' variant={"gray"} svg={true}>
                <svg width="24" height="16" viewBox="0 0 24 16" fill="none" xmlns="http://www.w3.org/2000/svg" className='mr-[10px]'>
